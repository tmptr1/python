...
### summary
Скрытое содержимое, разворачивается при нажатии на заоловок
```
<details>
...
```
Можно примерять стили к раскрытому варианту:
```
...
**Параметры:**
- **height**/**width** - размеры (px, %, vw, hw). Можно расчитывать: `calc(100vh - 75px)`
...
Чтобы задать **приоритет** параметру добавляется **!important**:  
`color: red !important;`  
_Нужно, например, если класс присваивается динамически_
...
- **first-child/last-child** - применяется к первому/последнему дочернему элементу
...
- **sticky** - фиксирует элемент относительно окна браузера, но при этом блок не залезает на другие блоки (например, можно делать фиксированный header, не перектывая часть content)
